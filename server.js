...
if (!fs.existsSync(WITHD_PATH))
  fs.writeFileSync(WITHD_PATH, JSON.stringify({ withdraws: [] }, null, 2));
...
app.post("/api/save-user", (req, res) => {
  try {
    ...
    if (!telegram_id || !ton_address)
      return res
        .status(400)
        .json({ ok: false, error: "telegram_id and ton_address required" });
    ...
    if (!user) {
      user = {
        ...
        balance: 0,
        miners: 0,
        ref_code: Math.random().toString(36).slice(2, 9),
        created_at: new Date().toISOString(),
        last_mine: Math.floor(Date.now() / 1000),
      };
      db.users.push(user);
    } else {
...
app.post("/api/withdraw", (req, res) => {
  try {
    ...
    if (!telegram_id || !amount || !to_address)
      return res.status(400).json({ ok: false });
...
app.post("/api/mine-tick", (req, res) => {
  try {
    ...
    const perMinerPerMinute = 0.0001;
    const minersCount = Math.max(1, user.miners || 0);
